...
### 1.1 Si vous n'êtes pas développeur

Vous n'avez pas besoin de connaître tout le vocabulaire interne pour utiliser
vibebackbone.

Le plus utile, au départ, est de garder ce cycle simple en tête :

1. Décrire ce que vous voulez faire.
2. Demander à l'agent de faire un intake VBB.
3. Faire planifier avant de modifier quoi que ce soit.
4. Vérifier avant de conclure.
5. Faire un handoff si vous faites une pause.

Les phrases courtes à copier sont dans la section 7.6. Si vous voulez juste
avancer sans dériver, commencez par là.
...
### 7.6 Usage quotidien — actionneurs courts

Quand vous voulez piloter vite, dites une phrase courte et laissez l'agent
ouvrir la bonne voie. Les formulations ci-dessous sont copiables telles
quelles.

#### Démarrer un chantier

- "Fais un intake VBB."
- "Analyse le contexte avant de coder."
- "Vérifie si le RICO est suffisant."

#### Reprendre un chantier

- "Reprends depuis le dernier handoff."
- "Charge `docs/SESSION.md` et vérifie l'état réel du dépôt."
- "Résume l'état du chantier avant de continuer."

#### Avancer vite

- "Fais une quick-task VBB."
- "Corrige uniquement ce point."
- "Ne change rien hors scope."

#### Avancer proprement

- "Lance une structured-task."
- "Fais un plan avant exécution."
- "Vérifie les impacts avant modification."
...
- "Fais un handoff."
- "Prépare une reprise rapide."
- "Mets à jour `docs/SESSION.md` sans closeout final."

#### Finir un chantier

- "Fais un closeout."
- "Clôture le chantier proprement."
- "Vérifie que le worktree est propre."

### 7.7 Handoff ≠ Closeout

Ces deux actions ne servent pas au même moment.

- `Handoff` = pause, transmission, reprise prévue.
- `Closeout` = clôture d'un chantier terminé.

En cas de doute :

- si le chantier continue, faites un `handoff`
- si le chantier est fini, faites un `closeout`

### 7.8 Cycle normal d'un projet

Le cycle le plus simple à retenir est celui-ci :

1. Je démarre → intake / contexte / RICO
2. Je comprends → audit ou analyse
3. Je prépare → plan
4. Je modifie → execution / quick-task / structured-task
5. Je vérifie → review / tests / `git status`
6. Je fais une pause → handoff
7. Je reprends → reprise depuis le handoff + vérification du dépôt
8. Je termine → closeout + worktree propre

### 7.9 Hygiène de travail

La discipline quotidienne est simple :

- évitez de laisser un worktree sale ;
- avant une pause longue, faites un `handoff` ;
- avant la fin d'un chantier, faites `review` puis `closeout` ;
- avant de reprendre, lisez le handoff puis vérifiez la réalité du dépôt ;
- ne supposez jamais que le handoff est la source de vérité absolue.

...
4. **Bootstrap and document hierarchy:** VBB Core invariants -> applicable
   repository-local operational `AGENTS.md` -> `docs/CONTEXT.md` ->
   `docs/PILOTAGE.md` -> `docs/PROJECT_MODE.md` -> `docs/SESSION.md` ->
   `docs/AUDIT_STATUS.md`. Local contracts cannot modify VBB governance; see
...
2. Before project state, deterministically check `$PWD/AGENTS.md`, otherwise
   `AGENTS.md` at the effective Git root; read the one contract found and
   report its Git state. `NONE` preserves normal VBB behavior; an
   unsafe/unreadable contract stops bootstrap. The Core verifier, when its
   checkout is available, is `tools/vbb-local-agents.py`.
3. Read the relevant governance files in hierarchy order.
4. Read `docs/SESSION.md` when present, even though it is local/gitignored.
5. Read `docs/AUDIT_STATUS.md` for active blockers before touching the domain.
6. Resume from documented state instead of inventing a new workflow.

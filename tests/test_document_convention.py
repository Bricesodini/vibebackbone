...
def run(root: Path, *args):
    return subprocess.run(
        [sys.executable, str(LINTER), str(root), *args], capture_output=True, text=True
    )
...
def test_invalid_metadata_taxonomies_and_version_fail(tmp_path):
    bad = (
        ...
        .replace("tags: [documentation]", "tags: [unknown-tag]")
        .replace("status: active", "status: ready")
    )
...
def test_legacy_and_historical_inconsistencies_fail(tmp_path):
    ...
    (root / "docs/OLD_TEMPLATE.md").write_text(
        VALID.replace("type: reference", "type: template"), encoding="utf-8"
    )
    (root / "docs/active.md").write_text(
        VALID.replace("status: active", "status: historical"), encoding="utf-8"
    )
    result = run(root)
    assert result.returncode != 0

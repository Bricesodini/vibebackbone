...
def test_ledger_cannot_launder_a_post_enforcement_run(tmp_path):
    ...
    """
    birth = gcg.ADVERSARIAL_ENFORCEMENT_BIRTH
...
def test_ledger_is_honored_for_a_pre_enforcement_run(tmp_path):
    ...
    from defect.
    """
    before = gcg.ADVERSARIAL_CUTOFF.replace(hour=gcg.ADVERSARIAL_CUTOFF.hour + 1)
    assert before < gcg.ADVERSARIAL_ENFORCEMENT_BIRTH
...
def test_open_run_is_out_of_scope_not_a_failure(tmp_path):
    ...
    """
    after = gcg.ADVERSARIAL_ENFORCEMENT_BIRTH
    ...
    assert result.category == gcg.OUT_OF_SCOPE
    assert result.category not in gcg.BLOCKING
